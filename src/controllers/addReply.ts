...
async function addReply(req: Request, res: Response) {
  ...
  try {
    ...
    const result = await messages.updateOne({ _id: new ObjectID(id) }, { $push: { replies: { _id: id, content: reply, name: replier, createTime } } })
    if (result) {
      res.json(dataWrapper({id: id.toString(), createTime}))
    } else {
